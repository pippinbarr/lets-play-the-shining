...
class DriveState extends FlxState
{
	...
	override public function create():Void
	{
		...
		superTitleText.setFormat(AssetPaths.Commodore__ttf, 24, 0xFF2255FF, "center");
		superTitleText.y = 60;
		...
		titleText.setFormat(AssetPaths.Commodore__ttf, 48, 0xFF2255FF, "center");
		...
		subTitleText.setFormat(AssetPaths.Commodore__ttf, 18, 0xFF2255FF, "center");
		// subTitleText.y = 60;
	...
	private function checkTriggers():Void
	{
		...
		if (car.overlaps(leftTrigger))
		{
			...
			if (completed)
				nextRoad = "r_u";
			if (arrivalTrigger != leftTrigger || totalScreens > MIN_TRAVEL_SCREENS)
				travelScreens++;
		...
		else if (car.overlaps(rightTrigger))
		{
			car.x = 0 + 2 * car.width;
			nextRoad = "l_u";
			if (arrivalTrigger != rightTrigger || totalScreens > MIN_TRAVEL_SCREENS)
				travelScreens++;
		...
		else if (car.overlaps(upTrigger))
		{
			...
			if (arrivalTrigger != upTrigger || totalScreens > MIN_TRAVEL_SCREENS)
				travelScreens++;
		...
		else if (car.overlaps(downTrigger))
		{
			car.y = 0 + 2 * car.width;
			nextRoad = "l_u";
			if (arrivalTrigger != downTrigger || totalScreens > MIN_TRAVEL_SCREENS)
				travelScreens++;
		...
		if (travelScreens >= MIN_TRAVEL_SCREENS - 1)
			completed = true;
	...
	private function checkTitle():Void
	{
		...
		if (titleText.y <= superTitleText.y + superTitleText.height)
		{
			titleText.y = superTitleText.y + superTitleText.height;
	...
	private function displaySuperTitle(t:FlxTimer):Void
	{
		...
		new FlxTimer().start(2, displaySubTitle);
	}
