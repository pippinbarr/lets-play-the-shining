...
class ThrowState extends FlxState
{
	...
	override public function create():Void
	{
		...
		// jack.x = 240;
		jack.x = FlxG.width / 2 - jack.width * 0.5;
		jack.y = 364;
	...
	{
		if (jack.x + jack.width * 0.5 >= FlxG.width)
			jack.x = FlxG.width - jack.width * 0.5;
		if (jack.x <= leftWall.x + leftWall.width * 1.4)
			jack.x = leftWall.x + leftWall.width * 1.4;
