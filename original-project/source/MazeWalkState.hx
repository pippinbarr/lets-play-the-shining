...
class MazeWalkState extends FlxState
{
	...
	private function moveLinear():Void
	{
		...
		{
			if (wendyDanny.facing == FlxObject.LEFT) wendyDanny.animation.play("walk-left");
			else wendyDanny.animation.play("walk-right");
		}
		else
		{
			if (wendyDanny.facing == FlxObject.LEFT) wendyDanny.animation.play("idle-left");
			else wendyDanny.animation.play("idle-right");
		}
	}
	...
	private function checkTriggers():Void
	{
		...
		if (wendyDanny.overlaps(leftTrigger))
		{
			...
			changeMaze = true;
			if (arrivalTrigger != leftTrigger) mazeScreens++;
		...
		else if (wendyDanny.overlaps(rightTrigger))
		{
			...
			changeMaze = true;
			if (arrivalTrigger != rightTrigger) mazeScreens++;
		...
		else if (wendyDanny.overlaps(upTrigger))
		{
			...
			changeMaze = true;
			if (arrivalTrigger != upTrigger) mazeScreens++;
		...
		else if (wendyDanny.overlaps(downTrigger))
		{
			...
			changeMaze = true;
			if (arrivalTrigger != downTrigger) mazeScreens++;
		...
		if (mazeScreens >= MIN_MAZE_SCREENS) completed = true;

		if (!changeMaze) return;
