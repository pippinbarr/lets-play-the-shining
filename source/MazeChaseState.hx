...
class MazeChaseState extends FlxState
{
	...
	override public function create():Void
	{
		...
		limpTimer = new FlxTimer();
		limpTimer.finished = true;

		jackMove = new FlxPoint(0,0);

		// new FlxTimer(SLOW_JACK_TIME,slowJack);
	...
	private function moveLinear():Void
	{
		...
		{
			if (jackSpeed >= 100) jack.animation.play("walk10");
			else if (jackSpeed >= 80) jack.animation.play("walk8");
			else if (jackSpeed >= 60) jack.animation.play("walk6");
			else jack.animation.play("walk4");
	...
	private function slowJack(t:FlxTimer):Void
	{
		...
		if (jack.velocity.x < 0) jack.velocity.x = -jackSpeed;
		else if (jack.velocity.x > 0) jack.velocity.y = jackSpeed;
		else if (jack.velocity.y < 0) jack.velocity.y = -jackSpeed;
		else if (jack.velocity.y > 0) jack.velocity.y = jackSpeed;
