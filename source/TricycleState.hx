...
class TricycleState extends FlxState
{
	...
	private function moveLinear():Void
	{
		...
		{
			if (danny.facing == FlxObject.LEFT) danny.animation.play("ride-left");
			else if (danny.facing == FlxObject.RIGHT) danny.animation.play("ride-right");
			danny.scale.x = 1;
		}
		else if (danny.velocity.y != 0)
		{
			if (danny.velocity.y < 0) danny.animation.play("ride-up");
			else danny.animation.play("ride-down");

			if (danny.facing == FlxObject.LEFT) danny.scale.x = 1;
			else if (danny.facing == FlxObject.RIGHT) danny.scale.x = -1;
		}
		else
		{
	...
	private function checkTriggers():Void
	{
		...
		if (danny.overlaps(leftTrigger))
		{
			...
			changeCorridor = true;
			if (arrivalTrigger != leftTrigger) corridorScreens++;
		...
		else if (danny.overlaps(rightTrigger))
		{
			...
			changeCorridor = true;
			if (arrivalTrigger != rightTrigger) corridorScreens++;
		...
		else if (danny.overlaps(upTrigger))
		{
			...
			changeCorridor = true;
			if (arrivalTrigger != upTrigger) corridorScreens++;
		...
		else if (danny.overlaps(downTrigger))
		{
			...
			changeCorridor = true;
			if (arrivalTrigger != downTrigger) corridorScreens++;
		...
		else if (danny.overlaps(endTrigger) && movementEnabled && checkEndTrigger)
		{
			...
			new FlxTimer(2,flickerTwins);

			// new FlxTimer(2,startTitle);
		}


		if (corridorScreens >= MIN_CORRIDOR_SCREENS) completed = true;

		if (!changeCorridor) return;
	...
	private function checkCarpet():Void
	{
		...
		if (FlxG.pixelPerfectOverlap(danny,wood) && !FlxG.pixelPerfectOverlap(danny,carpet) &&
